...
def test_angle_penalties():
    ...
    assert (
        u.atoms[ix_cc].name == "C"
    ), f"Expected C, got {u.atoms[ix_cc].name}"
    assert (
        u.atoms[ix_n].name == "N"
    ), f"Expected N, got {u.atoms[ix_n].name}"
    assert (
        u.atoms[ix_oc].name == "O"
    ), f"Expected O, got {u.atoms[ix_oc].name}"
    c_alpha = u.select_atoms(f"name CA and same residue as index {ix_cc}")[0]
    ix_ca = int(c_alpha.index)
    assert (
        u.atoms[ix_ca].name == "CA"
    ), f"Expected CA, got {u.atoms[ix_ca].name}"
    c_carbonyl = u.atoms[ix_cc]
    o_carbonyl = u.atoms[ix_oc]
    n_peptide = u.atoms[ix_n]
    water_os = u.select_atoms(
        f"name OW and resname SOL and around 5 index {ix_cc}"
    )
